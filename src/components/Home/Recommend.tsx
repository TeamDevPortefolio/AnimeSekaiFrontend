...
export default function Recommend({ anime }: Recommendations) {
    ...
    const list = anime.map((item, index) => (
        <a className='card_anime' key={item.documentId} onClick={() => Open(item)}>
            {item.imgTitle.url && <Image className='card_anime__title' alt='' src={`${process.env.NEXT_PUBLIC_API_URL}${item.imgTitle.url}`} width={100} height={50} />}
            {item.thumbnail.url && <Image className='card_anime__image' ref={imageRef} alt='' src={`${process.env.NEXT_PUBLIC_API_URL}${item.thumbnail.url}`} width={240} height={150} draggable="false" />}
            <span>{item.name ?? 'undefined'} </span>
        </a>
    ))
    useEffect(() => {
        fetch('http://localhost:1337/api/view/increment', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({ animeId: 5 }),
          });
    }, [open])
